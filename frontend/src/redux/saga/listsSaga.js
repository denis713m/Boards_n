...
export function* createList(action) {
    ...
    try{
        ...
        const newList = {name:action.payload.name, user: action.payload.user, id:uuidv4(), boardId:action.payload.board}
        saveListToStorage(newList);
        console.log(newList);
        yield put({type: types.LIST_CREATE_SUCCESS, data: newList});
    }
    catch (e) {
        yield put({type: types.LIST_OPERATION_ERROR, error: e});
    }
 };
...
export function* deleteList(action) {
    ...
    try{
        ...
        const stateList = list.lists.filter(element => ((element.id !== action.payload.list) 
                                            || (element.user !== action.payload.user)));   
        yield put({type: types.LIST_DELETE_SUCCESS, data:stateList});
    }
    catch (e) {
        yield put({type: types.LIST_OPERATION_ERROR, error: e.response});
    }
 }
